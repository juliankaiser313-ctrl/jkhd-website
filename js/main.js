...
(function () {
  const targets = document.querySelectorAll(
    ".section-head, .card, .pipeline-step, .split > *, .cta-band, .contact-box, .prose, .stat, .compare-row, .pull-quote, .timeline-item"
  );
  if (!targets.length) return;

  targets.forEach((el) => el.classList.add("reveal"));

  // Karten und Kennzahlen im Raster leicht versetzt einblenden
  document.querySelectorAll(".card-grid, .stat-grid").forEach((grid) => {
